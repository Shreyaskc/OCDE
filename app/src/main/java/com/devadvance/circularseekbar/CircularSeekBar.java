...
public class CircularSeekBar extends View {
    ...
    protected static final float DEFAULT_POINTER_RADIUS = 15f;
    protected static final float DEFAULT_POINTER_HALO_WIDTH = 0f;
    protected static final float DEFAULT_POINTER_HALO_BORDER_WIDTH = 0f;
    protected static final float DEFAULT_CIRCLE_STROKE_WIDTH = 20f;
    protected static final float DEFAULT_START_ANGLE = 270f; // Geometric
							     // (clockwise,
							     // relative to 3
							     // o'clock)
    protected static final float DEFAULT_END_ANGLE = 270f; // Geometric
							   // (clockwise,
							   // relative to 3
							   // o'clock)
    protected static final int DEFAULT_MAX = 100;
    protected static final int DEFAULT_PROGRESS = 0;
    protected static final int DEFAULT_CIRCLE_COLOR = 0xFF87ACDB;// Color.DKGRAY;
    protected static final int DEFAULT_CIRCLE_PROGRESS_COLOR = 0xFF87ACDB;// =
									  // Color.argb(235,
									  // 74,
									  // 138,
									  // 255);
    protected static final int DEFAULT_POINTER_COLOR = Color.argb(135, 74, 138,
	    255);// Color.argb(235, 74, 138,255);
    protected static final int DEFAULT_POINTER_HALO_COLOR = Color.argb(135, 74,
	    138, 255);// Color.argb(135, 74,138, 255);
    protected static final int DEFAULT_POINTER_HALO_COLOR_ONTOUCH = Color.argb(
	    135, 74, 138, 255);
    protected static final int DEFAULT_CIRCLE_FILL_COLOR = Color.TRANSPARENT;
    protected static final int DEFAULT_POINTER_ALPHA = 100;
    ...
    /**
     * {@code Paint} instance used to draw the center of the pointer. Note: This
     * is broken on 4.0+, as BlurMasks do not work with hardware acceleration.
     */
    protected Paint mPointerPaint;

    /**
     * {@code Paint} instance used to draw the halo of the pointer. Note: The
     * halo is the part that changes transparency.
     */
    protected Paint mPointerHaloPaint;

    /**
     * {@code Paint} instance used to draw the border of the pointer, outside of
     * the halo.
     */
    ...
    /**
     * Start angle of the CircularSeekBar. Note: If mStartAngle and mEndAngle
     * are set to the same angle, 0.1 is subtracted from the mEndAngle to make
     * the circle function properly.
     */
    protected float mStartAngle;

    /**
     * End angle of the CircularSeekBar. Note: If mStartAngle and mEndAngle are
     * set to the same angle, 0.1 is subtracted from the mEndAngle to make the
     * circle function properly.
     */
    ...
    /**
     * Holds the color value for {@code mPointerHaloPaint} before the
     * {@code Paint} instance is created.
     */
    ...
    /**
     * Holds the color value for {@code mCircleFillPaint} before the
     * {@code Paint} instance is created.
     */
    protected int mCircleFillColor = DEFAULT_CIRCLE_FILL_COLOR;

    /**
     * Holds the color value for {@code mCircleProgressPaint} before the
     * {@code Paint} instance is created.
     */
    ...
    /**
     * Distance (in degrees) that the the circle/semi-circle makes up. This
     * amount represents the max of the circle in degrees.
     */
    ...
    /**
     * If true, then the user can specify the X and Y radii. If false, then the
     * View itself determines the size of the CircularSeekBar.
     */
    protected boolean mCustomRadii;

    /**
     * Maintain a perfect circle (equal x and y radius), regardless of view or
     * custom attributes. The smaller of the two radii will always be used in
     * this case. The default is to be a circle and not an ellipse, due to the
     * behavior of the ellipse.
     */
    protected boolean mMaintainEqualCircle;

    /**
     * Once a user has touched the circle, this determines if moving outside the
     * circle is able to change the position of the pointer (and in turn, the
     * progress).
     */
    protected boolean mMoveOutsideCircle;

    /**
     * Used for enabling/disabling the lock option for easier hitting of the 0
     * progress mark.
     */
     ...
     * counter clockwise. Makes it easier to hit the 0 progress mark.
     */
    protected boolean lockAtStart = true;

    /**
     * Used for when the user moves beyond the end of the circle when moving
     * clockwise. Makes it easier to hit the 100% (max) progress mark.
     */
     ...
     * angle. Used when touching the CircularSeekBar.
     */
    protected float cwDistanceFromStart;

    /**
     * Represents the counter-clockwise distance from {@code mStartAngle} to the
     * touch angle. Used when touching the CircularSeekBar.
     */
     ...
     * touch angle. Used when touching the CircularSeekBar. Currently unused,
    ...
    /**
     * The previous touch action value for {@code cwDistanceFromStart}. Used
     ...
     * to the touch angle. Used when touching the CircularSeekBar.
     */
    protected float ccwDistanceFromPointer;

    /**
     * True if the user is moving clockwise around the circle, false if moving
     * counter-clockwise. Used when touching the CircularSeekBar.
     */
    protected boolean mIsMovingCW;

    /**
     * The width of the circle used in the {@code RectF} that is used to draw
     * it. Based on either the View width or the custom X radius.
     */
    protected float mCircleWidth;

    /**
     * The height of the circle used in the {@code RectF} that is used to draw
     * it. Based on either the View width or the custom Y radius.
     */
    protected float mCircleHeight;

    /**
     * Represents the progress mark on the circle, in geometric degrees. This is
     * not provided by the user; it is calculated;
     */
    ...
    /**
     * True if user touch input is enabled, false if user touch input is
     * ignored. This does not affect setting values programmatically.
     */
	...
	mCircleColor = attrArray.getColor(
		R.styleable.CircularSeekBar_circle_color, DEFAULT_CIRCLE_COLOR);
	...
	lockEnabled = attrArray.getBoolean(
		R.styleable.CircularSeekBar_lock_enabled, DEFAULT_LOCK_ENABLED);

	// Modulo 360 right now to avoid constant conversion
	mStartAngle = ((360f + (attrArray.getFloat(
		(R.styleable.CircularSeekBar_start_angle), DEFAULT_START_ANGLE) % 360f)) % 360f);
	mEndAngle = ((360f + (attrArray.getFloat(
		(R.styleable.CircularSeekBar_end_angle), DEFAULT_END_ANGLE) % 360f)) % 360f);
	...
	mCircleProgressGlowPaint.setMaskFilter(new BlurMaskFilter(
		(5f * DPTOPX_SCALE), BlurMaskFilter.Blur.NORMAL));
     ...
     * mTotalCircleDegrees to this value.
     */
    protected void calculateTotalDegrees() {
	mTotalCircleDegrees = (360f - (mStartAngle - mEndAngle)) % 360f; // Length
									 // of
									 // the
									 // entire
									 // circle/arc
    ...
    /**
     * Calculate the degrees that the progress represents. Also called the sweep
     * angle. Sets mProgressDegrees to that value.
     */
     ...
     * degrees. Sets mPointerPosition to that value.
     */
    ...
    /**
     * Set the progress of the CircularSeekBar. If the progress is the same,
     * then any listener will not receive a onProgressChanged event.
     * 
     * @param progress
	...
	if (mMaintainEqualCircle) { // Applies regardless of how the values were
				    // determined
     ...
     * 
     * @return Boolean value of true if the pointer locks at zero and max, false
     *         if it does not.
     */
     ...
     * 
     * @param boolean value. True if the pointer should lock at zero and max,
	...
	float touchEventRadius = (float) Math
		.sqrt((Math.pow(distanceX, 2) + Math.pow(distanceY, 2)));

	float minimumTouchTarget = MIN_TOUCH_TARGET_DP * DPTOPX_SCALE; // Convert
								       // minimum
								       // touch
								       // target
								       // into
	...
	float outerRadius = Math.max(mCircleHeight, mCircleWidth)
		+ additionalRadius; // Max outer radius of the circle, including
				    // the minimumTouchTarget or wheel width
	float innerRadius = Math.min(mCircleHeight, mCircleWidth)
		- additionalRadius; // Min inner radius of the circle, including
				    // the minimumTouchTarget or wheel width
	...
	case MotionEvent.ACTION_DOWN:
	    ...
	    float pointerRadiusDegrees = (float) ((mPointerRadius * 180) / (Math.PI * Math
		    .max(mCircleHeight, mCircleWidth)));
		...
		return false;
		    ...
		    && (touchEventRadius <= outerRadius)) { // If the user is
							    // touching near the
							    // circle
	...
	case MotionEvent.ACTION_MOVE:
		...
		} else if ((mMoveOutsideCircle)
			|| (touchEventRadius <= outerRadius)) {
	...
	mPointerHaloColorOnTouch = savedState
		.getInt("mPointerHaloColorOnTouch");
    ...
    /**
     * Listener for the CircularSeekBar. Implements the same methods as the
     * normal OnSeekBarChangeListener.
     */
     ...
     * 
     * @param alpha
     *            the alpha of the pointer (0..255) when touched
     */
    ...
    /**
     * Set the max of the CircularSeekBar. If the new max is less than the
     * current progress, then the progress will be set to zero. If the progress
     * is changed as a result, then any listener will receive a
     ...
     * 
     * @param boolean value. True if user touch input is to be accepted, false
     *        if user touch input is to be ignored.
     */
     ...
     * 
     * @return Boolean value of true if user touch input is accepted, false if
     *         user touch input is ignored.
     */
